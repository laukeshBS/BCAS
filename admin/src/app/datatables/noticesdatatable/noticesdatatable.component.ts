...
import { NoticesService } from '../../services/notices.service';  // Import the service
...
@Component({
  selector: 'app-Noticesdatatable',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './noticesdatatable.component.html',
  styleUrl: './noticesdatatable.component.css'
})
export class NoticesdatatableComponent {
  events: any[] = [];
  selectedEvent: any = {};
  ...
  lang_code = 'en'; 
  selectedFile: any;
  selectedFileError: string | null = null; // Initialized with nullcurrentPage: number = 1; // Current page for pagination
  ...
  loadList(): void {
    ...
    this.NoticesService.allList(this.limit, this.lang_code, this.currentPage).subscribe(data => {
  ...
  editEvent(id: number): void {
    this.NoticesService.getEvent(id).subscribe(data => {
  ...
  saveEvent(): void {
    ...
    formData.append('important', this.selectedEvent.important);
    ...
    this.NoticesService.storeEvent(formData).subscribe(
      (event: HttpEvent<any>) => {
  ...
  modifyEvent(): void {
    ...
    this.NoticesService.updateEvent(this.selectedEvent.id, formData).subscribe(
      (event: HttpEvent<any>) => {
          this.loadList(); // Refresh the list of events
  ...
  deleteEvent(id: number): void {
    if (confirm('Are you sure you want to delete this event?')) {
      this.NoticesService.deleteEvent(id).subscribe(() => {

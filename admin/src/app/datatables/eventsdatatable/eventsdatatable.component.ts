...
export class EventsdatatableComponent {
  ...
  loadList(): void {
    ...
    this.EventsService.allList(this.limit, this.lang_code, this.currentPage).subscribe(data => {
  ...
  saveEvent(): void {
    ...
    if (this.fileToUpload) {
      const validFileTypes = ['image/jpeg', 'image/png', 'image/gif', 'image/jpg']; // Example types
